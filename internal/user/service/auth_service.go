...
func (s AuthSvc) isValidUserId(t *jwt.Token, id string) bool {
	if t == nil {
		return false
	} else {
...
func (s AuthSvc) getUserByNickname(u string) (*model.User, error) {
...
func NewAuthService(rep repo.UserRepository, log *log.Logger, configs *user.AppConfig) AuthService {
	return &AuthSvc{
		repository: rep,
		log:        log,
		configs:    configs,
